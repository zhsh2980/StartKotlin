...
import kotlinx.android.synthetic.main.activity_simple_data_struct.*
 ...
 * 数据类型
 */
class SimpleDataStructActivity : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_simple_data_struct)
        onBtnClick()//点击事件
        dataStruct()//数据结构
        controlCodes()//控制语句
    }
    ...
    private fun controlCodes() {
        ...
        // **可** 为空  只能调用下面两种  即使初始化时赋值不为空
        var strCanNull: String? = "hahaah"
        strCanNull.isNullOrBlank()
        strCanNull.isNullOrEmpty()

        //为空不能直接调用 .length等 api
//        strCanNull.length
        //检查不为空才可以调用
        strCanNull?.apply {
            strCanNull.length
